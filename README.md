...
This is a natural evolution of passwords, which simply brings a one dimensional password (a line of characters) into 2 dimensions (width and height), which is also known as a matrix, aka an array of arrays. This method of construction, I believe, is the correct way of using a password "phrase" by having memorable yet complex passwords.
...
This example implementation is a 64x64 matrix with 8 bits (two hex characters) per cell.  This means it is an 8 bits x 64 length x 64 width matrix result which is 32,768 bits in total.  2 to the power of 32,768 bits, 2n ** 32768n (New BigNum Exponential JavaScript), is a number 9,865 digits long (bring it on quantum).
...
I have included two initial 512-bit hash functions:

1. Blake2b: for new yet strong + fast hashes (this is my recent favourite hash even though Blake3 is now out, but time has not yet given Blake3 time to prove its strength, SipHash isn't big enough for my liking but is also one of my favourites)
2. SHA-512: for tried and true and so far unbroken for many years, it is a bit slower in software but included for preference, flexibility and comparison and as an example on how to add future algorithms. Some systems also have hardware/accelerated/integrated/pre-existing SHA-512 computation that can be used for this algorithm in alternative implementations.

Other implementations can use any size hash and matrix so long as both of their sizes match and the hash fuction can be used as a keyed HMAC.
...
The result of this final matrix hash is the actual final result, without conversion for storage or comparison, I envisioned; and is still a two dimentional bit-map password.
